...
main().then(console.log('🎉 connected to database successfully')).catch(error => console.error(error));
...
const initApplication = async () => {
  ...
  const server = new ApolloServer({
    ...
    resolvers,
    context: setContext,
    introspection: environmentVariablesConfig.environment === ENVIRONMENT.PRODUCTION ? false : true,
    // Set to "true" only in development mode
    plugins: environmentVariablesConfig.environment === ENVIRONMENT.PRODUCTION ? [ApolloServerPluginLandingPageDisabled()] : [requestDevLogger, ApolloServerPluginLandingPageGraphQLPlayground()],

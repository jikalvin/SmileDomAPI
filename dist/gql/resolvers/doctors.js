...
export default {
  ...
  Mutation: {
    updatePatientInfo: async (parent, {
      input
    }, context) => {
      const {
        fullName,
      ...
      const doctor = await context.di.model.Doctors.findOne({
        uuid: user.uuid
      });
      if (fullName) {
        await context.di.model.Doctors.findOneAndUpdate({
          uuid: user.uuid
        }, {
          $set: {
            fullName: fullName
      ...
      return await context.di.model.Doctors.findOne({
        uuid: user.uuid
      });
    }
  }

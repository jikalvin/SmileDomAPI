...
export default {
  ...
  Query: {
    ...
    getChatMessages: async (parent, args, context) => {
      context.di.authValidation.ensureThatUserIsLogged(context);
      const sender = await context.di.model.Users.findOne({
        uuid: args.senderId
      });
      const receiver = await context.di.model.Users.findOne({
        uuid: args.receiverId
      });
      return await context.di.model.Messages.find({
        sender: sender._id,
        receiver: receiver._id
      });
    },
    getCalls: async (parent, args, context) => {
      return await context.di.model.Calls.find({
        $or: [{
          caller: args.userId
        }, {
  ...
  Mutation: {
    ...
    makeCall: async (parent, args, context) => {
      context.di.authValidation.ensureThatUserIsLogged(context);
      const caller = await context.di.model.Users.findOne({
        uuid: args.callerId
      });
      const receiver = await context.di.model.Users.findOne({
        uuid: args.receiverId
    ...
    endCall: async (parent, args, context) => {
      context.di.authValidation.ensureThatUserIsLogged(context);
      return await context.di.model.Calls.findByIdAndUpdate(args.callId, {
        endTime: new Date()
      }, {
        new: true
      });

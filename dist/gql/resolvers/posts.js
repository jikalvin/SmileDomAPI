...
export default {
  ...
  Query: {
    ...
    getLikes: async (parent, {
      ...
      context.di.authValidation.ensureThatUserIsLogged(context);
      const pst = await context.di.model.Posts.findOne({
        puid
      });
      const sortCriteria = {
        post: pst
      };
      return context.di.model.Likes.find({
        sortCriteria
      }).lean();
    },
    getComments: async (parent, {
      ...
      return context.di.model.Comments.find({
        sortCriteria
      }).lean();
    }
  },
  Mutation: {
    ...
    updatePost: async (parent, {
      ...
      return await context.di.model.Posts.findByIdAndUpdate(puid, {
        title,
        content
      }, {
        new: true
    ...
    createLike: async (parent, {
      puid
    }, context) => {
      context.di.authValidation.ensureThatUserIsLogged(context);
      const user = await context.di.authValidation.getUser(context);
      const post = await context.di.model.Posts.findOne({
        puid
      });
